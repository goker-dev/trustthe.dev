...
import { NoteDtoReadable } from '@/api/kodkafa' // Adjust path
import {
  AUTHOR_NAME,
  AUTHOR_URL,
  BASE_URL,
...
import {
  ...
  getPageDescription,
  getPageTitle,
} from './metadata.utils' // Adjust path
/**
...
export function metadataGenerator(
  data: NoteDtoReadable | undefined,
  options: Options = {},
): Metadata {
  ...
  const description =
    options.description ||
    getPageDescription(data?.markdown, DEFAULT_DESCRIPTION)
  ...
  const metaData: Metadata = {
    ...
    // Add other tags like robots, verification, etc. as needed
