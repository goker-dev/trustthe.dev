...
function generateThemeTokens(
  ...
  if (!isDark) {
    finalBackgroundL =
      lightModeBgLMin + normalizedContrast * (lightModeBgLMax - lightModeBgLMin)
  } else {
    finalBackgroundL =
...
export default function ThemeCustomizerButton() {
  ...
  useEffect(() => {
    ...
    if (storedOriginalRaw) {
      try {
        const parsedStoredOriginal = JSON.parse(storedOriginalRaw) as ThemeModes
    ...
    if (userStoredConfigsRaw) {
      try {
        const parsedUserConfigs = JSON.parse(userStoredConfigsRaw) as ThemeModes
